...
if (file_exists('dz7_base.txt')){                                  // если такой файл существует
    $ad_cookie = unserialize(file_get_contents('dz7_base.txt'));        // расшифровываем данные и записываем их в $ad_cookie
...
if (isset($_POST['confirm_add'])) {                          // кнопка добавить
   ...
   save_for_ad_cookie($ad_cookie);
    ...
    $ad_cookie[$click_id]['id'] = $click_id;           // присваиваем элементу массива с текущим объявлением его номер в ключ id для возможности его сохранения
